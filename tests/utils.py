...
class BackgroundState(StateBaseClass):
    def __init__(self, lx=lx, ly=ly, state_vars=DEFAULT_STATE_VARS):
        ...
        self.x = np.linspace(-lx / 2.0, lx / 2, int(lx / dx) + 1)
...
class FakeCloudState(StateBaseClass):
    ...
    def r(self, t):
        ...
        def r_fn(t):
            ...
            if t_ < 0.0:
                ...
                return self.r_min + dr * np.min([1.0, t_ / self.tau])
            else:
                return 0.0
    ...
    def _wrap_range(l, p, p_min):
        return p - l * np.round((p - l / 2.0 - p_min) / l)
    ...
    def _get_mask(self, t):
        ...
        dist = np.sqrt(xx ** 2.0 + yy ** 2.0) * np.sign(t - self.t0)
    ...
    def _get_z_top(self, t):
        ...
        def z_top_fn(t):
            ...
            if t_ < 0.0:
                ...
                return z_min + (z_max - z_min) * np.max(
                    [0.0, np.min([t_ / self.tau, 1.0])]
                )
    ...
    def _get_core(self, t):
        ...
        return xr.where(m, self.core_max * (1.0 - dist / r), np.nan)

    def _get_lwp(self, t):
        m, r, dist = self._get_mask(t)
        return xr.where(
            np.logical_and(dist < r, r > 0.0), self.lwp_max * (1.0 - dist / r), np.nan
        )
...
class FakeSimulation:
    ...
    def generate_output(self, dt=2 * 60):
        t_ = np.arange(0.0, self.t_max + 120.0, dt)
    ...
    def output(self, dt_sim=2 * 60, keep_data=False, base_name="testdata"):
        return SyntheticDatasetGenerator(
            sim=self, dt_sim=dt_sim, keep_data=keep_data, base_name=base_name
        )


class SyntheticDatasetGenerator:
    def __init__(self, sim, dt_sim, base_name, keep_data=False):
        self.sim = sim
        self.dt_sim = dt_sim
    ...
    def __enter__(self):
        ...
        ds_input.attrs["base_name"] = self.base_name
